...
class AgentBuildService
{
    ...
    /**
     * The agent's `claude -p` can exit non-zero even after a COMPLETE build (a flaky final verification
     * step, an MCP hiccup, etc.). If it actually produced the compiled Vite assets, accept the build —
     * don't throw away ~15 min of correct work over a misleading exit code.
     */
    private function fullstackBuilt(string $dir): bool
    {
        return is_file($dir.'/public/build/manifest.json')
            && count(glob($dir.'/database/migrations/*.php') ?: []) > 0;
    }
    ...
    private function writeFullstackContext(string $dir, array $ctx): void
    {
        ...
        - **INICIO PÚBLICO (crítico para sitios de cara al cliente):** si el proyecto lo visitará el PÚBLICO (institucional, donaciones, viajes, tienda, landing, servicios), la ruta `/` DEBE ser la LANDING pública real (hero + secciones + contenido del negocio + llamados a la acción), visible SIN iniciar sesión. NUNCA redirijas `/` a `/login` ni a `/dashboard` — el template trae esa redirección por defecto, QUÍTALA en `routes/web.php` y pon ahí la página pública de inicio. El panel de administración va aparte en `/admin` (o `/dashboard`) tras login. Las demás páginas de cara al cliente (nosotros, causas, contacto, etc.) también públicas.
